...
@section('content')
  <div class="min-h-screen flex flex-col items-center justify-center bg-gray-50 py-10 px-2">
    ...
    <div class="w-full max-w-2xl bg-white rounded-2xl shadow p-8">
      <h1 class="text-2xl font-bold mb-8 text-center">Editar Categoria</h1>
      <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="flex flex-col space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
          ...
          <select name="active" id="active"
            ...
            <option value="1" {{ old('active', $category->active) == 1 ? 'selected' : '' }}>Ativa</option>
            <option value="0" {{ old('active', $category->active) == 0 ? 'selected' : '' }}>Inativa</option>
          </select>
        </div>
