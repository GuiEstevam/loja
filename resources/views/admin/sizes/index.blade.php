    ...
    <div class="bg-white shadow rounded-lg p-6 md:p-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-6">
        <nav class="flex items-center gap-2 text-base md:text-lg text-gray-500">
          ...
          <span>/</span>
          <span class="text-gray-700 font-semibold">Tamanhos</span>
        </nav>
      ...
      <h1 class="text-3xl font-bold mb-2 text-center md:text-left">Gerenciar Tamanhos</h1>
      <p class="text-gray-600 mb-6 text-center md:text-left">Visão geral e gestão de todos os tamanhos</p>

      <!-- Filtros -->
      <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4 items-end mb-6">
        <div>
          <label class="block text-xs text-gray-500 mb-1">Buscar:</label>
          <input type="text" name="search" value="{{ request('search') }}" placeholder="Nome do tamanho..."
        ...
        <table class="min-w-full divide-y divide-gray-200 text-lg">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-center font-semibold">Ativo</th>
          ...
          <tbody class="bg-white divide-y divide-gray-100">
            @forelse($sizes as $size)
              <tr class="hover:bg-gray-50 transition">
                <td class="px-4 py-3 font-semibold">{{ $size->name }}</td>
                <td class="px-4 py-3 text-center">
                  <span
                    class="px-3 py-2 rounded-full text-base font-bold
                    {{ $size->active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                    {{ $size->active ? 'Sim' : 'Não' }}
                  </span>
                </td>
                <td class="px-4 py-3 text-center">
                  <div class="flex gap-4 justify-center items-center">
                    <a href="{{ route('admin.sizes.show', $size) }}" class="text-indigo-600 hover:text-indigo-900"
                      title="Ver produtos">
                    ...
                    <a href="{{ route('admin.sizes.edit', $size) }}" class="text-blue-600 hover:text-blue-900"
                      title="Editar">
                      <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <form action="{{ route('admin.sizes.destroy', $size) }}" method="POST" class="inline"
                      onsubmit="return confirm('Remover este tamanho?')">
                      @csrf @method('DELETE')
            ...
            @empty
              <tr>
                <td colspan="3" class="py-8 text-center text-gray-500 text-lg">Nenhum tamanho encontrado.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
