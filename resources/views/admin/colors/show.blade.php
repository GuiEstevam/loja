@extends('layouts.app')
@section('title', 'Produtos da Cor: ' . $color->name)
    ...
    <div class="bg-white shadow rounded-lg p-6 md:p-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-6">
        <nav class="flex items-center gap-2 text-base md:text-lg text-gray-500">
          ...
          <span>/</span>
          <span class="text-gray-700 font-semibold">{{ $color->name }}</span>
        </nav>
      ...
      <h1 class="text-3xl font-bold mb-2 text-center md:text-left">
        Produtos com a cor "{{ $color->name }}"
        <span class="inline-block w-6 h-6 rounded-full border ml-2 align-middle"
          style="background: {{ $color->hex_code ?? '#eee' }};"></span>
      </h1>
      <p class="text-gray-600 mb-6 text-center md:text-left">Todos os produtos que possuem esta cor</p>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-lg">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="py-4 px-6 text-left font-semibold">Tamanhos</th>
