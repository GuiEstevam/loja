...
@section('content')
  <div class="min-h-screen flex flex-col items-center justify-center bg-gray-50 py-10 px-2">
    ...
    <div class="w-full max-w-2xl bg-white rounded-2xl shadow p-8">
      <h1 class="text-2xl font-bold mb-8 text-center">Nova Marca</h1>
      <form method="POST" action="{{ route('admin.brands.store') }}" class="flex flex-col space-y-6">
        @csrf
        ...
        <div>
          ...
          <input type="text" name="slug" id="slug" value="{{ old('slug') }}"
            ...
            required autocomplete="off">
          <small class="text-gray-500">URL amigável, sem espaços. Ex: nike, adidas</small>
          @error('slug')
            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
          @enderror
        </div>
